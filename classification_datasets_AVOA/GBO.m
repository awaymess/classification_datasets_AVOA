...
for i=1:nP
    Cost(i) = fobj(X(i,:),mlpConfig);      % Calculate the Value of Objective Function
    %Cost(i) = test_functions(X(i,:),F_index,nV);
    %Cost(i) = cec17_func(X(i,:)',F_index);
end
...
for it=1:MaxIt
    ...
    for i=1:nP
        ...
        Xnew_Cost = fobj(Xnew,mlpConfig);
        %Xnew_Cost = test_functions(Xnew,F_index,nV);
        %Xnew_Cost = cec17_func(Xnew',F_index);
